...
$form_args = array(
	...
	'comment_field'        =>  '<p class="push-down-15"><label for="message">' . __( 'Message' , 'organique_wp' ) . ( $req ? ' <span class="warning">*</span>' : '' ) . '</label><textarea class="form-control form-control--contact form-control--big" id="comment" name="comment" rows="7" aria-required="true" placeholder="Your Comment goes here." required>' . '</textarea></p>',
	'fields' => array(
		...
		'url' => '<p class="push-down-15"><label for="url">' . __( 'Website' , 'organique_wp' ) . ( $req ? ' <span class="warning">*</span>' : '' ) . '<input class="form-control  form-control--contact" id="url" name="url" type="text" value="' . esc_attr(  $commenter['comment_author_url'] ) . '" size="30" /></label></p>',
	)
);
